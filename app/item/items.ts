...
export class Inventory {
  ...
  public delete(id: Number): Promise<any> {
    return this.db.execSQL("DELETE FROM items WHERE id = " + id);
  }

  public getById(id: Number): Promise<any> {
    return new Promise((resolve, reject) => {
      this.db.all("SELECT * FROM items where id = " + id).then(
        rows => {
          let item = {
            id: rows[0][0],
            name: rows[0][1],
  ...
  public fetch(): Promise<any> {
    return new Promise((resolve, reject) => {
      this.db.all("SELECT * FROM items").then(
        rows => {
          let items = [];
          for (var row in rows) {
            items.push({
